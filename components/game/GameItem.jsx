     ...
     <div className="card-top img-fit-cover">
      ...
      <StarRating rating = { gameItem?.rating} />
       <div className="ratings-count">
         { gameItem?.ratings_count} <BsStarFill className='ms-1' size={ 12 } color='#FFD700'/>
       </div>
     </div>
     <div className="card-bottom">
      <h4 className="text-white text-uppercase card-title">
        ...
        <div className="block-wrap d-flex align-items-end justify-content-between">
          ...
          <Link to = {`/games/${gameItem?.id}`} className='card-button text-uppercase'> see more <BiSolidHandDown size ={12}/> </Link>
        </div>
     </div>
    </GameItemWrapper>
  )
